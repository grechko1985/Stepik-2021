...
s = [int(input('Введите число: '))]                  # создаем список, в который записываем первое число
while sum(s) != 0:                                   # до тех пор, пока сумма всех чисел не равна 0
    s.append(int(input('Введите число снова: ')))    # добавляем новое значение в СПИСОК, которое вводится с клавиатуры
print(sum([i**2 for i in s]))                        # по окончанию цикла печатаем сумму квадратов